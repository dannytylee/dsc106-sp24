...
var app = (function () {
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			t1 = space();
    			input = element("input");
    			add_location(label, file, 188, 2, 5585);
    			attr_dev(input, "id", "slider");
    			attr_dev(input, "type", "range");
    			attr_dev(input, "min", "1");
    			attr_dev(input, "max", "1440");
    			add_location(input, file, 189, 2, 5617);
    			attr_dev(div, "class", "overlay");
    			add_location(div, file, 187, 4, 5561);
    			add_location(main, file, 186, 0, 5550);
    		},
    ...
    function scaleColorTrafficVolume(arrivalRatio, departureRatio) {
    	const scaleColor = d3.scaleLinear().domain([0, arrivalRatio, departureRatio, 1]).range(["#ffffff", "blue", "red", "#000000"]);
    	return scaleColor(arrivalRatio, departureRatio);
    }

    function instance($$self, $$props, $$invalidate) {
    	let { $$slots: slots = {}, $$scope } = $$props;
    	validate_slots('App', slots, []);
    	mapboxgl.accessToken = "pk.eyJ1IjoiZGFubnl0eWxlZSIsImEiOiJjbHZ6dnlzZXYwcGFiMmpudjEycTk2N2pkIn0.LSxeT-jgTB699U_KrEEUDA";
    	...
    	function update_station_markers() {
    		station_markers.transition().duration(500).attr("r", function (d) {
    			let trafficVolume = arrivals[d["station_id"]] + departures[d["station_id"]];
    			return scaleRadiusTrafficVolume(trafficVolume);
    		}).style("fill", function (d) {
    			let trafficVolume = arrivals[d["station_id"]] + departures[d["station_id"]];
    			let arrivalRatio = arrivals[d["station_id"]] / trafficVolume;
    			let departureRatio = departures[d["station_id"]] / trafficVolume;
    			return scaleColorTrafficVolume(arrivalRatio, departureRatio);
    		});
    	}
